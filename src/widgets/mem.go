...
func (self *MemWidget) update() {
	...
	if err != nil {
		...
		self.Labels["Main"] = fmt.Sprintf("%3.0f%% %5.1f%s/%.0f%s", mainMemory.UsedPercent, mainMemoryUsedBytes, mainMemoryUsedMagnitude, mainMemoryTotalBytes, mainMemoryTotalMagnitude)
	}
	...
	if err != nil {
		...
		self.Labels["Swap"] = fmt.Sprintf("%3.0f%% %5.1f%s/%.0f%s", swapMemory.UsedPercent, swapMemoryUsedBytes, swapMemoryUsedMagnitude, swapMemoryTotalBytes, swapMemoryTotalMagnitude)
	}
}
